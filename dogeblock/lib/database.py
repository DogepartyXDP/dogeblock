...
from dogeblock.lib import config, cache, util
from dogeblock.lib.processor import RollbackProcessor
...
def reset_db_state():
    ...
    config.mongo_db.app_config.update({}, {
        'db_version': config.DB_VERSION,  # dogeblockd database version
        'running_testnet': config.TESTNET,
        'running_regtest': config.REGTEST,
        'dogepartyd_db_version_major': None,
        'dogepartyd_db_version_minor': None,
        'dogepartyd_running_testnet': None,
        'dogepartyd_running_regtest': None,
...
def rollback(max_block_index):
    ...
    (which will get a new dp_latest_block from dogepartyd and resume as appropriate)
    """
