...
from dogeblock.lib import config, util
...
def get_doge_supply(normalize=False, at_block_index=None):
    """returns the total supply of DOGE (based on what dogecoind says the current block height is)"""
...
def pubkey_to_address(pubkey_hex):
    ...
    address_prefix = b'\x71' if (config.TESTNET or config.REGTEST) else b'\x1e'
    return encoding.public_pair_to_dogecoin_address(public_pair, compressed=compressed, address_prefix=address_prefix)


def dogecoind_rpc(command, params):
...
def get_doge_balance(address, confirmed=True):
...
def getaddressinfo(address):
    ...
    # TODO: This code was disabling dogewallet and it really doesn't has reason to exist
...
def broadcast_tx(signed_tx_hex):
    return dogecoind_rpc('sendrawtransaction', [signed_tx_hex])
