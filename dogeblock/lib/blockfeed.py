"""
blockfeed: sync with and process new blocks from dogeparty-server
"""
import re
import os
import sys
...
from dogeblock.lib import config, util, blockchain, cache, database
from dogeblock.lib.processor import MessageProcessor, MempoolMessageProcessor, BlockProcessor, CaughtUpProcessor
...
def fuzzy_is_caught_up():
    """We don't want to give users 525 errors or login errors if dogeblockd/dogeparty-server is in the process of
    getting caught up, but we DO if dogeblockd is either clearly out of date with the blockchain, or reinitializing its database"""
    if not config.state['caught_up']:
        return False
    if not config.state['dp_backend_block_index'] or not config.state['my_latest_block']['block_index']:
        return False
    if config.state['my_latest_block']['block_index'] < config.state['dp_backend_block_index'] - 1:  # "fuzzy" part
        return False
    return True


def process_dp_blockfeed():
    # initialize state
    config.state['cur_block'] = {'block_index': 0, }  # block being currently processed
    config.state['my_latest_block'] = {'block_index': 0}  # last block that was successfully processed by dogeblockd
    config.state['last_message_index'] = -1  # initialize (last processed message index)
    config.state['dp_latest_block_index'] = 0  # last block that was successfully processed by dogeparty
    config.state['dp_backend_block_index'] = 0  # the latest block height as reported by the dpd blockchain backend
    config.state['dp_caught_up'] = False  # whether dogeparty-server is caught up to the backend (e.g. dogecoind)
                ...
                for function in MempoolMessageProcessor.active_functions():
                    ...
                    if result == 'ABORT_THIS_MESSAGE_PROCESSING' or result == 'continue':
                        ...
                        raise Exception(
                            ...
                            (function, result))
        logger.debug("Mempool refresh: {} entries retrieved from dogeparty-server, {} new".format(len(new_txs['result']) if new_txs else '??', (len(new_txs['result']) - num_skipped_tx) if new_txs else '??'))
    ...
    def parse_block(block_data):
        ...
        logger.info("Block: %i of %i [message height=%s]" % (
            ...
            config.state['dp_backend_block_index']
            if config.state['dp_backend_block_index'] else '???',
    ...
    if(app_config.count() == 0 or
       ...
       app_config[0]['running_regtest'] != config.REGTEST):
        if app_config.count():
            logger.warn("dogeblockd database version UPDATED (from %i to %i) or testnet/regtest setting changed (from %s to %s, or %s to %s). REBUILDING FROM SCRATCH ..." % (
            ...
            logger.warn("dogeblockd database app_config collection doesn't exist. BUILDING FROM SCRATCH...")
        ...
        if my_latest_block:
            # remove any data we have for blocks higher than this (would happen if dogeblockd or mongo died
    ...
    # avoid contacting dogeparty-server (on reparse, to speed up)
    autopilot = False
    autopilot_runner = 0
    iteration = 0
    ...
    # start polling dogeparty-server for new blocks
    dp_running_info = None
    while True:
        iteration += 1
        if iteration % 10 == 0:
            logger.info(
        ...
        logger.debug(
            "iteration: ap %s/%s, dp_latest_block_index: %s, my_latest_block: %s" % (autopilot, autopilot_runner,
                                                                                     config.state['dp_latest_block_index'], config.state['my_latest_block']['block_index']))

        if not autopilot or autopilot_runner == 0:
            try:
                dp_running_info = util.jsonrpc_api("get_running_info", abort_on_error=True, use_cache=False)['result']
            except Exception as e:
                logger.warn("Cannot contact dogeparty-server (via get_running_info): {}".format(e))
                time.sleep(3)
                continue

        # wipe our state data if necessary, if dogeparty-server has moved on to a new DB version
        wipeState = False
        updatePrefs = False

        # Checking appconfig against old running info (when batch-fetching) is redundant
        if    app_config['dogepartyd_db_version_major'] is None \
           or app_config['dogepartyd_db_version_minor'] is None \
           or app_config['dogepartyd_running_testnet'] is None \
           or app_config['dogepartyd_running_regtest'] is None:
            logger.info("Updating version info from dogeparty-server")
            updatePrefs = True
        elif dp_running_info['version_major'] != app_config['dogepartyd_db_version_major']:
            logger.warn(
                "dogeparty-server MAJOR DB version change (we built from %s, dogeparty-server is at %s). Wiping our state data." % (
                    app_config['dogepartyd_db_version_major'], dp_running_info['version_major']))
            wipeState = True
            updatePrefs = True
        elif dp_running_info['version_minor'] != app_config['dogepartyd_db_version_minor']:
            logger.warn(
                "dogeparty-server MINOR DB version change (we built from %s.%s, dogeparty-server is at %s.%s). Wiping our state data." % (
                    app_config['dogepartyd_db_version_major'], app_config['dogepartyd_db_version_minor'],
                    dp_running_info['version_major'], dp_running_info['version_minor']))
            wipeState = True
            updatePrefs = True
        elif dp_running_info.get('running_testnet', False) != app_config['dogepartyd_running_testnet']:
            logger.warn("dogeparty-server testnet setting change (from %s to %s). Wiping our state data." % (
                app_config['dogepartyd_running_testnet'], dp_running_info['running_testnet']))
            wipeState = True
            updatePrefs = True
        elif dp_running_info.get('running_regtest', False) != app_config['dogepartyd_running_regtest']:
            logger.warn("dogeparty-server regtest setting change (from %s to %s). Wiping our state data." % (
                app_config['dogepartyd_running_regtest'], dp_running_info['running_regtest']))
            wipeState = True
            updatePrefs = True
        if wipeState:
        ...
        if updatePrefs:
            app_config['dogepartyd_db_version_major'] = dp_running_info['version_major']
            app_config['dogepartyd_db_version_minor'] = dp_running_info['version_minor']
            app_config['dogepartyd_running_testnet'] = dp_running_info['running_testnet']
            app_config['dogepartyd_running_regtest'] = dp_running_info['running_regtest']
        ...
        # work up to what block dogepartyd is at
        try:
            if dp_running_info['last_block']:  # should normally exist, unless dogeparty-server had an error getting it
                assert dp_running_info['last_block']['block_index']
                config.state['dp_latest_block_index'] = dp_running_info['last_block']['block_index']
            elif dp_running_info['db_caught_up']:
                config.state['dp_latest_block_index'] = dp_running_info['dogecoin_block_count']
            else:
                assert False
        except:
            logger.warn(
                "dogeparty-server not returning a valid last processed block (probably is reparsing or was just restarted)."
                + " Waiting 3 seconds before trying again... (Data returned: %s, we have: %s)" % (
                    dp_running_info, config.state['dp_latest_block_index']))
            time.sleep(3)
            continue

        config.state['dp_backend_block_index'] = dp_running_info['dogecoin_block_count']
        config.state['dp_caught_up'] = dp_running_info['db_caught_up']

        if config.state['my_latest_block']['block_index'] < config.state['dp_latest_block_index']:
            # need to catch up
            config.state['caught_up'] = False

            # TODO: Autopilot and autopilot runner are redundant
            if config.state['dp_latest_block_index'] - config.state['my_latest_block']['block_index'] > 500:
            ...
            try:
                block_data = cache.get_block_info(
                    cur_block_index,
                    prefetch=min(100, (config.state['dp_latest_block_index'] - config.state['my_latest_block']['block_index'])),
            ...
            try:
                ...
                # dogeparty-server might have gone away...
                my_latest_block = config.mongo_db.processed_blocks.find_one(sort=[("block_index", pymongo.DESCENDING)])
                if my_latest_block:
                    database.rollback(my_latest_block['block_index'])

                # disable autopilot this next iteration to force us to check up against dogeparty-server
            ...
            if config.state['dp_latest_block_index'] - cur_block_index < config.MAX_REORG_NUM_BLOCKS:  # only when we are near the tip
                clean_mempool_tx()
        elif config.state['my_latest_block']['block_index'] > config.state['dp_latest_block_index']:
            # should get a reorg message. Just to be on the safe side, prune back MAX_REORG_NUM_BLOCKS blocks
            # before what dogepartyd is saying if we see this
            logger.error(
                "Very odd: Ahead of dogeparty-server with block indexes! Pruning back %s blocks to be safe."
                % config.MAX_REORG_NUM_BLOCKS)
            database.rollback(config.state['dp_latest_block_index'] - config.MAX_REORG_NUM_BLOCKS)
        else:
            if config.IS_REPARSING:
            ...
            # ...we may be caught up (to dogepartyd), but dogepartyd may not be (to the blockchain). And if it isn't, we aren't
            config.state['caught_up'] = config.state['dp_caught_up']

            # this logic here will cover a case where we shut down dogeblockd, then start it up again quickly...
            ...
            if config.state['last_message_index'] == -1 or config.state['my_latest_block']['block_index'] == 0:
                if config.state['last_message_index'] == -1:
                    config.state['last_message_index'] = dp_running_info['last_message_index']
                if config.state['my_latest_block']['block_index'] == 0:
                    config.state['my_latest_block']['block_index'] = dp_running_info['last_block']['block_index']
            ...
            publish_mempool_tx()
            time.sleep(2)  # dogeblockd itself is at least caught up, wait a bit to query again for the latest block from dpd
