...
from dogeblock.lib import config, cache, database, util, blockchain, blockfeed, messages
from dogeblock.lib.processor import API
...
def serve_api():
    ...
    # the server side, this makes it easier for 3rd party wallets (i.e. not Dogewallet) to fully be able to
    # use dogeblockd to not only pull useful data, but also load and store their own preferences, containing
    ...
    def get_chain_block_height():
        # DEPRECIATED 1.5
        return config.state['dp_backend_block_index']
    ...
    def get_chain_address_info(addresses, with_uxtos=True, with_last_txn_hashes=4):
        ...
        for address in addresses:
            ...
            result['addr'] = address
            result['info'] = info
            result['block_height'] = config.state['dp_backend_block_index']
    ...
    def get_raw_transactions(address, start_ts=None, end_ts=None, limit=1000):
        ...
        def get_address_history(address, start_block=None, end_block=None):
            ...
            address_dict['dogepays'] = util.call_jsonrpc_api(
                "get_dogepays",
                {'filters': [{'field': 'source', 'op': '==', 'value': address}, {'field': 'destination', 'op': '==', 'value': address}],
                 'filterop': 'or',
                 'order_by': 'block_index',
        ...
        # make API call to dogeparty-server to get all of the data for the specified address
        txns = []
    ...
    @API.add_method
    def proxy_to_dogepartyd(method='', params=[]):
        if method == 'sql':
    ...
    def handle_get():
        ...
        #"ping" dogepartyd to test
        dp_s = time.time()
        dp_result_valid = True
        try:
            dp_status = util.call_jsonrpc_api("get_running_info", abort_on_error=True, use_cache=False)['result']
        except:
            dp_result_valid = False
        dp_e = time.time()

        #"ping" dogeblockd to test, as well
        db_s = time.time()
        db_result_valid = True
        db_result_error_code = None
        payload = {
            "id": 0,
            "jsonrpc": "2.0",
            "method": "is_ready",
        ...
        try:
            ...
            db_result_valid = False
            db_result_error_code = "GOT EXCEPTION: %s" % e
        else:
            if r.status_code != 200:
                db_result_valid = False
                db_result_error_code = "GOT STATUS %s" % r.status_code if r else 'COULD NOT CONTACT'
            db_result = r.json()
            if 'error' in db_result:
                db_result_valid = False
                db_result_error_code = "GOT ERROR: %s" % db_result['error']
        db_e = time.time()

        result = {
            'dogeparty-server': 'OK' if dp_result_valid else 'NOT OK',
            'dogeparty-server_ver': '%s.%s.%s' % (
                dp_status['version_major'], dp_status['version_minor'], dp_status['version_revision']) if dp_result_valid else '?',
            'dogeparty-server_last_block': dp_status['last_block'] if dp_result_valid else '?',
            'dogeparty-server_last_message_index': dp_status['last_message_index'] if dp_result_valid else '?',
            'dogeparty-server_caught_up': config.state['dp_caught_up'],
            'dogeparty-server_check_elapsed': dp_e - dp_s,

            'dogeblock': 'OK' if db_result_valid else 'NOT OK',
            'dogeblock_ver': config.VERSION,
            'dogeblock_check_elapsed': db_e - db_s,
            'dogeblock_error': db_result_error_code,
            'dogeblock_last_message_index': config.state['last_message_index'],
            'dogeblock_caught_up': blockfeed.fuzzy_is_caught_up(),
            'dogeblock_cur_block': {'block_hash': config.state['cur_block'].get('block_hash', '??'),
                                       'block_index': config.state['cur_block'].get('block_index', '??')},
            'dogeblock_last_processed_block': {'block_hash': config.state['my_latest_block'].get('block_hash', '??'),
                                                  'block_index': config.state['my_latest_block'].get('block_index', '??')},
        }

        response_code = 200
        # error if we couldn't make a successful call to dogeparty-server or dogeblock's API (500)
        if not dp_result_valid or not db_result_valid:
            response_code = 500
            result['ERROR'] = "dogeparty-server_api_contact_error"
        # error 510 if the dogeparty-server last block is more than 1 block behind backend
        elif not result['dogeparty-server_caught_up']:
            response_code = 510
            result['ERROR'] = "dogeparty-server_not_caught_up"
        # error 511 if the dogeblock last block is more than 1 block behind dogeparty-server
        elif not result['dogeblock_caught_up']:
            response_code = 511
            result['ERROR'] = "dogeblock_not_caught_up"
        else:
            result['ERROR'] = None
