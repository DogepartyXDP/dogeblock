...
from dogeblock.lib import config, blockchain, database
...
def decorate_message(message, for_txn_history=False):
    ...
    if message['_category'] in ['orders', 'order_matches', ]:
        message['_doge_below_dust_limit'] = (
            ('forward_asset' in message and message['forward_asset'] == config.DOGE and message['forward_quantity'] <= config.ORDER_DOGE_DUST_LIMIT_CUTOFF)
            or ('backward_asset' in message and message['backward_asset'] == config.DOGE and message['backward_quantity'] <= config.ORDER_DOGE_DUST_LIMIT_CUTOFF)
        )
...
def decorate_message_for_feed(msg, msg_data=None):
    """This function takes a message from dogepartyd's message feed and mutates it a bit to be suitable to be
    sent through the dogeblockd message feed to an end-client"""
    if not msg_data:
...
def get_address_cols_for_entity(entity):
    if entity in ['debits', 'credits']:
        ...
        return ['issuer', ]
    elif entity in ['sends', 'dividends', 'bets', 'cancels', 'orders', 'burns', 'broadcasts', 'dogepays']:
        return ['source', ]
