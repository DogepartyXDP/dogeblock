...
from dogeblock.lib.modules import ASSETS_PRIORITY_PARSE_ISSUANCE, ASSETS_PRIORITY_PARSE_DESTRUCTION, ASSETS_PRIORITY_BALANCE_CHANGE
...
def get_normalized_balances(addresses):
    """
    This call augments dogeparty's get_balances with a normalized_quantity field. It also will include any owned
...
def get_escrowed_balances(addresses):
    ...
    bindings = addresses + ['open', config.DOGE]
    ...
    sql = '''SELECT (tx0_address || '_' || forward_asset) AS source_asset, tx0_address AS address, forward_asset AS asset, SUM(forward_quantity) AS quantity
             ...
             GROUP BY source_asset'''.format(addresses_holder)
    bindings = addresses + [config.DOGE, 'pending']
    results += util.call_jsonrpc_api("sql", {'query': sql, 'bindings': bindings}, abort_on_error=True)['result']
    ...
    sql = '''SELECT source AS address, '{}' AS asset, SUM(wager_remaining) AS quantity
             ...
             GROUP BY address'''.format(config.XDP, addresses_holder)
    ...
    for asset in assets:
        ...
        if asset in [config.DOGE, config.XDP]:
            if asset == config.DOGE:
                supply = blockchain.get_doge_supply(normalize=False)
            else:
                supply = util.call_jsonrpc_api("get_supply", {'asset': config.XDP}, abort_on_error=True)['result']
...
def parse_destruction(msg, msg_data):
    ...
    if msg_data['status'] != 'valid':
        return
    if msg_data['asset'] != None and msg_data['asset'] == 'XDP':
        return  
...
def process_rollback(max_block_index):
    if not max_block_index:  # full reparse
        ...
        # create XDP and DOGE assets in tracked_assets
        for asset in [config.XDP, config.DOGE]:
            base_asset = {
                'asset': asset,
                'asset_longname': None,
                'owner': None,
