"""
Implements dogewallet support as a dogeblock plugin
...
from dogeblock.lib import config, util, blockfeed, blockchain, messages
from dogeblock.lib.processor import MessageProcessor, MempoolMessageProcessor, BlockProcessor, StartUpProcessor, CaughtUpProcessor, RollbackProcessor, API, start_task, CORE_FIRST_PRIORITY
from dogeblock.lib.modules import CWALLET_PRIORITY_PARSE_FOR_SOCKETIO, CWALLET_PRIORITY_PUBLISH_MEMPOOL

from dogeblock.lib.processor import startup
...
def _read_config():
    ...
    config_path = os.path.join(config.config_dir, 'dogewallet%s.conf' % config.net_path_part)
    logger.info("Loading config at: %s" % config_path)
    try:
        configfile.read(config_path)
        assert configfile.has_section('Default')
    except:
        logging.warn("Could not find or parse dogewallet%s.conf config file!" % config.net_path_part)
...
def handle_invalid(msg, msg_data):
    ...
    if status.startswith('invalid'):
        if config.state['dp_latest_block_index'] - config.state['my_latest_block']['block_index'] < config.MAX_REORG_NUM_BLOCKS:
...
def init():
    ...
    def download_geoip_data():
        ...
        if download:
            ...
            cmd = "cd '{}' && curl -L -O https://dogeparty.net/bootstrap/GeoLite2-City.mmdb.gz && gzip -d GeoLite2-City.mmdb.gz".format(config.data_dir)
    ...
    if not module_config['SUPPORT_EMAIL']:
        logger.warn("Support email setting not set: To enable, please specify an email for the 'support-email' setting in your dogeblockd.conf")
