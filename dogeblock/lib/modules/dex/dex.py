...
from dogeblock.lib import cache, config, util
...
def get_pairs_with_orders(addresses=[], max_pairs=12):
    ...
    for my_pair in my_pairs:
        ...
        if my_pair['pair'] == config.DOGE_TO_XDP:  # XDP/DOGE always in first
...
def get_pairs(quote_asset=config.XDP, exclude_pairs=[], max_pairs=12, from_time=None):
...
def get_quotation_pairs(exclude_pairs=[], max_pairs=12, from_time=None, include_currencies=[]):
    ...
    for currency in currencies:
        ...
        for currency_pair in currency_pairs:
            if currency_pair['pair'] == config.XDP_TO_DOGE:
    ...
    for currency in quote_assets:
        if len(top_pairs) < max_pairs:
            ...
            for currency_pair in currency_pairs:
                ...
                if currency_pair['pair'] == config.XDP_TO_DOGE:  # XDP/DOGE always in first
                    top_pairs.insert(0, top_pair)
                else:
                    top_pairs.append(top_pair)
                all_assets += [currency_pair['base_asset'], currency_pair['quote_asset']]

    if (config.DOGE in quote_assets) and (config.XDP_TO_DOGE not in [p['base_asset'] + '/' + p['quote_asset'] for p in top_pairs]):
        top_pairs.insert(0, {
            'base_asset': config.XDP,
            'quote_asset': config.DOGE
        })
        all_assets += [config.XDP, config.DOGE]
...
def get_market_orders(asset1, asset2, addresses=[], supplies=None, min_fee_provided=0.95, max_fee_required=0.95):
    ...
    for order in orders:
        ...
        exclude = False
        if order['give_asset'] == config.DOGE:
            try:
        ...
        elif order['get_asset'] == config.DOGE:
            try:
...
def get_assets_supply(assets=[]):
    ...
    if config.XDP in assets:
        supplies[config.XDP] = (util.call_jsonrpc_api("get_supply", {'asset': config.XDP})['result'], True)
        assets.remove(config.XDP)

    if config.DOGE in assets:
        supplies[config.DOGE] = (0, True)
        assets.remove(config.DOGE)
...
def get_markets_list(quote_asset=None, order_by=None):
    ...
    markets = []
    pairs = []
    currencies = [config.XDP, config.XDOGE] if not quote_asset else [quote_asset]
    ...
    for pair in pairs:
        ...
        if market['base_asset'] == config.XDP and market['quote_asset'] == config.DOGE:
