"""
Implements dogewallet asset-related support as a dogeblock plugin

DEPENDENCIES: This module requires the assets module to be loaded before it.

Python 2.x, as dogeblock is still python 2.x
"""
import os
import sys
import time
...
from dogeblock.lib import config, util, blockfeed, blockchain
from dogeblock.lib.modules import DEX_PRIORITY_PARSE_TRADEBOOK
from dogeblock.lib.processor import MessageProcessor, MempoolMessageProcessor, BlockProcessor, StartUpProcessor, CaughtUpProcessor, RollbackProcessor, API, start_task
...
def get_market_info_leaderboard(limit=100):
    ...
    assets_market_info_xdp = list(config.mongo_db.asset_market_info.find({}, {'_id': 0}).sort('market_cap_in_{}'.format(config.XDP.lower()), pymongo.DESCENDING).limit(limit))
    assets_market_info_doge = list(config.mongo_db.asset_market_info.find({}, {'_id': 0}).sort('market_cap_in_{}'.format(config.DOGE.lower()), pymongo.DESCENDING).limit(limit))
    assets_market_info = {
        config.XDP.lower(): [a for a in assets_market_info_xdp if a['price_in_{}'.format(config.XDP.lower())]],
        config.DOGE.lower(): [a for a in assets_market_info_doge if a['price_in_{}'.format(config.DOGE.lower())]]
    }
    # throw on extended info, if it exists for a given asset
    assets = list(set([a['asset'] for a in assets_market_info[config.XDP.lower()]] + [a['asset'] for a in assets_market_info[config.DOGE.lower()]]))
    extended_asset_info = config.mongo_db.asset_extended_info.find({'asset': {'$in': assets}})
    extended_asset_info_dict = {}
    for e in extended_asset_info:
        if not e.get('disabled', False):  # skip assets marked disabled
            extended_asset_info_dict[e['asset']] = e
    for r in (assets_market_info[config.XDP.lower()], assets_market_info[config.DOGE.lower()]):
        for a in r:
...
def _get_order_book(base_asset, quote_asset,
    ...
    if base_asset == config.DOGE or quote_asset == config.DOGE:
        extra_filters = [
    ...
    def get_o_pct(o):
        if o['give_asset'] == config.DOGE:  # NB: fee_provided could be zero here
            pct_fee_provided = float((D(o['fee_provided_remaining']) / D(o['give_quantity'])))
        else:
            pct_fee_provided = None
        if o['get_asset'] == config.DOGE:  # NB: fee_required could be zero here
    ...
    def make_book(orders, isBidBook):
        ...
        for o in orders:
            if o['give_asset'] == base_asset:
                if base_asset == config.DOGE and o['give_quantity'] <= config.ORDER_DOGE_DUST_LIMIT_CUTOFF:
                ...
                if quote_asset == config.DOGE and o['give_quantity'] <= config.ORDER_DOGE_DUST_LIMIT_CUTOFF:
                    continue  # filter dust orders, if necessary
...
def get_order_book_buysell(buy_asset, sell_asset, pct_fee_provided=None, pct_fee_required=None):
    ...
    if base_asset == config.DOGE:
        if buy_asset == config.DOGE:
    ...
    elif quote_asset == config.DOGE:
        assert base_asset == config.XDP  # only time when this is the case
        if buy_asset == config.DOGE:
...
def get_users_pairs(addresses=[], max_pairs=12):
    return dex.get_users_pairs(addresses, max_pairs, quote_assets=[config.XDP, config.XDOGE])
...
def parse_trade_book(msg, msg_data):
    ...
    if(msg['category'] == 'order_matches' and
       ((msg['command'] == 'update' and msg_data['status'] == 'completed') or  # for a trade with DOGE involved, but that is settled (completed)
        ('forward_asset' in msg_data and msg_data['forward_asset'] != config.DOGE and msg_data['backward_asset'] != config.DOGE)
        )
       ):  # or for a trade without DOGE on either end

        if msg['command'] == 'update' and msg_data['status'] == 'completed':
            # an order is being updated to a completed status (i.e. a DOGEpay has completed)
            tx0_hash, tx1_hash = msg_data['order_match_id'][:64], msg_data['order_match_id'][65:]
            # get the order_match this dogepay settles
            ...
            assert msg_data['status'] == 'completed'  # should not enter a pending state for non DOGE matches
        ...
        # don't create trade records from order matches with DOGE that are under the dust limit
        if((order_match['forward_asset'] == config.DOGE and
            order_match['forward_quantity'] <= config.ORDER_DOGE_DUST_LIMIT_CUTOFF)
           or (order_match['backward_asset'] == config.DOGE and
               order_match['backward_quantity'] <= config.ORDER_DOGE_DUST_LIMIT_CUTOFF)):
            logger.debug("Order match %s ignored due to %s under dust limit." % (order_match['tx0_hash'] + order_match['tx1_hash'], config.DOGE))
