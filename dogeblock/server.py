...
from dogeblock.lib import config, config_util, log, blockfeed, util, module, database
from dogeblock.lib.processor import messages, caughtup, startup  # to kick off processors
from dogeblock.lib.processor import StartUpProcessor
...
CONFIG_ARGS = [
    ...
    [('--backend-connect',), {'help': 'the hostname or IP of the backend dogecoind JSON-RPC server'}],
    ...
    [('--dogeparty-connect',), {'help': 'the hostname of the dogepartyd JSON-RPC server'}],
    [('--dogeparty-port',), {'type': int, 'help': 'the port used to communicate with dogepartyd over JSON-RPC'}],
    [('--dogeparty-user',), {'help': 'the username used to communicate with dogepartyd over JSON-RPC'}],
    [('--dogeparty-password',), {'help': 'the password used to communicate with dogepartyd over JSON-RPC'}],
    ...
    [('--rpc-port',), {'type': int, 'help': 'port on which to provide the dogeblockd JSON-RPC API'}],
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='dogeblock', description='dogeblock daemon')

    parser.add_argument(
        '-V', '--version', action='version', version="dogeblock %s" % config.VERSION)
    ...
    parser_server = subparsers.add_parser('server', help='Run dogeblock')
    parser_reparse = subparsers.add_parser('reparse', help='Reparse the dogeblock database')
    parser_enmod = subparsers.add_parser('enmod', help='Enable a module')
    parser_enmod.add_argument('module_path', type=str, help='Full Path of module to Enable relative to Dogeblockd directory')
    parser_dismod = subparsers.add_parser('dismod', help='Disable a module')
    parser_dismod.add_argument('module_path', type=str, help='Path of module to Disable relative to Dogeblockd directory')
    ...
    # Handle arguments
    logger.info("dogeblock command specified: {}".format(args.command))
    ...
    logger.info("dogeblock Version %s starting ..." % config.VERSION)
